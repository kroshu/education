...
class GetAgeServer(Node):
    ...
    def get_age_callback(self, request, response):
        ...
        today = date.today()
        response.age = today.year - birth.year - ((today.month, today.day) < (birth.month, birth.day))
        self.logger.info(f'For birth date {birth_string} sending back age: {response.age}')
